...
name_SEfit_Hvede='Hvede_SEfit_log_saveOK.mat';
load(name_SEfit_Hvede,'first_or_second','fitresult_se','gof_se','output_se','se_fit_mapp','fitresult_se_2','gof_se_2','output_se_2','se_fit_mapp_2', 'save_ydata','save_xdata','Mapp_cell','IP_flag_cell','t_center','depths_cell','GateWidth','coeff_alpha_Hvede','coeff_tauKWW_Hvede','coeff_beta_Hvede','rsquare_Hvede','current_cell','Rho_save','Res_save','file_in')
    ...
    for i=1:num_dp
        ...
        if isempty(fitresult_se{k,i})
            ...
            if rsquare_fit(k,i)<0.99 %0.998
                ...
                % ydata=se_fit_mapp_to_use{k,i}';
                % ydata_0=fitresult_se{k,i}.a*exp(-(xdata/fitresult_se{k,i}.b).^fitresult_se{k,i}.c); %solve size issues  
                ydata_0=coeff_alpha_Hvede(k,i)*exp(-(xdata/coeff_tauKWW_Hvede(k,i)).^coeff_beta_Hvede(k,i));   
                ydata=ydata_0';
                % std_average_data=save_ydata{k,depth_id0}(:,2);
                % std_residuals_SEfit=residuals_fit{k,depth_id0};
                % if std_residuals_SEfit>0
                %     ydata_std_pos=std_average_data+std_residuals_SEfit;
                %     ydata_std_neg=std_average_data;
                % elseif std_residuals_SEfit<0
                %     ydata_std_neg=std_average_data-std_residuals_SEfit;
                %     ydata_std_pos=std_average_data;
                % else
                %     warning(['residuals neither pos nor neg at depth=' num2str(save_depthdata(k,depth_id0)) 'm for file' num2str(k)])
                %     ydata_std_pos=std_average_data;
                %     ydata_std_neg=std_average_data;
                % end
                % ydata_rms_tot(k,depth_id0)=sqrt(sum(((ydata_std_pos+ydata_std_neg)/2).^2)/numobs_fit);
                % Mapp_avg=mean(ydata);
                % Mapp_std_tmp=ydata_rms_tot(k,depth_id0);
                % Mapp_std_rel(k,depth_id0)=Mapp_std_tmp/Mapp_avg;
            end
        end
        if isempty(ydata)
            ...
            tau_min=log10(min(save_xdata{k,i}));
            tau_max=log10(max(save_xdata{k,i}));
...
%% save variables
name_final='mat\Hvede_RTD_099_saveXk_dec24_short_interval.mat';
save(name_final,'sum_x','weightedTau_x','coeff_tau', 'coeff_x', 'save_ydata','save_xdata','coeff_alpha_Hvede','coeff_tauKWW_Hvede','coeff_beta_Hvede','rsquare_Hvede','file_in','current_cell','Rho_save','Res_save','rsquare_fit','rmse_fit','residuals_fit','resnorms_RTD','residuals_RTD')
