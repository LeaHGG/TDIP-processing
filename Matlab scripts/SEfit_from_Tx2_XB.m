...
coeff_alpha_Hvede=NaN(n,num_depth_ref);
coeff_tauKWW_Hvede=NaN(n,num_depth_ref);
coeff_beta_Hvede=NaN(n,num_depth_ref);
rsquare_Hvede=NaN(n,num_depth_ref);
first_or_second=NaN(n,num_depth_ref);
    ...
    for i=1:num_dp(k)
        ...
        if numel(xdata0)>10
            ...
            if numel(xdata0_excl)>10
                ...
                txt_title=[file_in{k}(1:12) '-DP:' num2str(i) '; I =' num2str(round(1000*current_cell{k,1}(i),0)) ' mA' '; R2_1 =' num2str(gof_se{k, i}.rsquare,3) '; R2_2 =' num2str(gof_se_2{k, i}.rsquare,3) ];
                ...
                plot(xdata0_noflag,ydata0_noflag,'*','Color',[1 0 0],'MarkerSize',7);
                txt_title=[file_in{k}(1:12) '-DP:' num2str(i) '; I =' num2str(round(1000*current_cell{k,1}(i),0)) ' mA' '; R2_1 =' num2str(gof_se{k, i}.rsquare,3)];
            ...
            if ~isempty(gof_se_2{k, i})                   
                if gof_se_2{k, i}.rsquare>gof_se{k, i}.rsquare
                    coeff_alpha_Hvede(k,i) = coeff_alpha_2(k,i);
                    coeff_tauKWW_Hvede(k,i) = coeff_tauKWW_2(k,i);
                    coeff_beta_Hvede(k,i) = coeff_beta_2(k,i);
                    rsquare_Hvede(k,i) = gof_se_2{k, i}.rsquare;
                    first_or_second(k,i)=2;
                end
            else
                coeff_alpha_Hvede(k,i) = coeff_alpha(k,i);
                coeff_tauKWW_Hvede(k,i) = coeff_tauKWW(k,i);
                coeff_beta_Hvede(k,i) = coeff_beta(k,i);
                rsquare_Hvede(k,i) = gof_se{k, i}.rsquare;
                first_or_second(k,i)=1;
            end
        else
...
% save mat file
name_final='mat\Hvede_SEfit_log_saveOK.mat';
